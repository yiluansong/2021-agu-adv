...
p_human <- levelplot(Human, par.settings = my.settings, at = cutpts, margin = FALSE, col.regions = rev(colorspace::heat_hcl(5)), main = "", 
                     # xlab = list(label = expression("Population density (persons km"^-2 * ")"), cex = 0.8), 
...
nonspatial <- function(data, response) {
  ...
  # nonsplm<-bayesLMRef(lm(response~population.density, spdata), n.samples=5000)
...
make_diag_plot <- function(response) {
  ...
  grid.arrange(
    grobs=list(annotate_figure(p7, top=text_grob("Nonspatial model"),fig.lab = label, fig.lab.pos = "top.left", fig.lab.face = "bold"),
