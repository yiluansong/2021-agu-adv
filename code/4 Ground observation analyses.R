...
rnpn::npn_phenophases() %>% filter(pheno_class_id==1)%>% arrange(phenophase_name)
...
for (i in 1:nrow(species_list)){
  npn_df <- rnpn::npn_download_site_phenometrics(request_source='YS',
...
for (i in 1:nrow(species_list)) {
  ...
  if (nrow(npn_df)>0) {
    ...
    npn_coords_sp<-SpatialPoints(coords=data[,c("lon", "lat")],
...
for (i in 1:length(species_list_common_name)) {
  ...
  coef<-data.frame(common_name=species_list_common_name[i],
...
### Plot
png(filename = "./figures/Figure 5_map.png",
...
p_trend<-ggplot()+
  ...
  scale_y_continuous(breaks=sqrt(c(0,4,16,36,64)),
                     labels=c(0,4,16,36,64))+
  scale_x_continuous(breaks=log(c(0,10,100,1000)+1),
