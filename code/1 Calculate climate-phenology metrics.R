...
for (season in season_list) {
  ...
  bg_Vc <- levelplot(Vc,
    par.settings = my.settings, at = cutpts, margin = FALSE, col.regions = rev(colorspace::heat_hcl(20)), main = "",
  ...
  p_anglediff <- levelplot(anglediff,
    par.settings = my.settings, at = cutpts, margin = FALSE, col.regions = col_pal_diverge, main = "",
  ...
  if (season == "year") {
    ...
    x_label_list <- list(
      c(
        expression(bolditalic(v)["MAT"]),
        expression(bolditalic(v)["MST"]),
        expression(bolditalic(v)["MFT"])
      ),
      c(
        expression(bolditalic(v)["GSL"]),
...
crop_layer <- function(i, var) {
  ...
  if (i == 2) {
    temp <- crop(layer, extent(-180, 180, 50, 70))
  }
  if (i == 3) {
    temp <- crop(layer, extent(-180, 180, 30, 50))
  }
  if (i == 4) {
    temp <- layer * Eumask
  }
...
y_label_list <- c(
  expression(bolditalic(v)["MAT"] * " (km yr"^-1 * ")"),
